...
public class ViewDetailController extends HttpServlet {
    ...
    private static final String VIEW_DETAIL_PAGE = "viewDetail.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Products p = dao.getByID(productID);
            List<Products> listAnother = dao.getAnotherProductExcept(productID);
            request.setAttribute("PRODUCT_DETAIL", p);
            request.setAttribute("LIST_ANOTHER_PRODUCT", listAnother);
        } catch (Exception e) {
            log("Error at ViewDetailController: " + e.toString());
        } finally {
            request.getRequestDispatcher(VIEW_DETAIL_PAGE).forward(request, response);
        }
    }
